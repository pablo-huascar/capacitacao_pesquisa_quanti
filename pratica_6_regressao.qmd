...
-   Nos modelos em que todos os pressupostos da regressão forem atendidos:
    -   Monte a equação de regressão (só inclua na equação os preditores que forem significativos no modelo);
    -   Escreva a interpretação do intercepto $(b_{0})$ e dos parâmetros de inclinação $(b_{1})$; 
    - Avalie se o intercepto tem uma interpretação prática ou somente matemática
    -   Calcule manualmente os valores previstos para os casos solicitado(s);
    -   Escreva o relato dos resultados conforme modelo apresentado a seguir.

## Modelo para o relato da regressão

Avaliamos os pressupostos da regressão linear, confirmando a ausência de multicolinearidade (VIF < 2; tolerância > 0,3) e a independência dos resíduos (Durbin-Watson = 2,01; $p = 0,80$). No entanto, detectamos 104 potenciais outliers e encontramos heterocedasticidade (teste Breusch-Pagan: $\chi^2 = 47,1;$ $p < 0,001$) e distribuição não normal dos resíduos (teste Shapiro-Wilk: $W = 0,92$; $p < 0,001$). Além disso, as variâncias não foram homogêneas entre os níveis de escolaridade (teste de Levene: $\chi^2 = 15,96$; $p < 0,01$) e as categorias dos estilos de liderança do diretor escolar (teste de Levene: $\chi^2 = 8,68$; $p = 0,03$).
